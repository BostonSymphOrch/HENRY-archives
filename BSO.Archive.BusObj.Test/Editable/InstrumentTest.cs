...
namespace BSO.Archive.BusObj.Test
{
    ...
    public class InstrumentTest
    {
        ...
        {
            Instrument instrument1 = Instrument.GetInstrumentByID(7);
            if (instrument1.IsNew)
                instrument1.InstrumentID = 7;
            BsoArchiveEntities.Current.Save();
            Instrument instrument2 = Instrument.GetInstrumentByID(7);
        ...
        public void GetInstrumentFromNodeTest()
        {
            ...
            Assert.IsTrue(instrument.InstrumentID == 3 && instrument.Instrument1 == "TestInstr");
        }

        [TestMethod]
        public void UpdateInstrumentTest()
        {
            Instrument instrument = Instrument.GetInstrumentByID(4);
            Assert.IsTrue(instrument.InstrumentID == 4);
            Assert.IsTrue(instrument.Instrument1 == "Bassoon");
            DateTime lastModified = instrument.ModifiedOn;

            var instrumentID = Helper.CreateXElement(Constants.Artist.artistInstrumentIDElement, "4");
            var instrument1 = Helper.CreateXElement(Constants.Artist.artistInstrumentElement, "Adage Test");
            ...
            Assert.IsTrue(instrument.Instrument1 == "Adage Test");
        }
    }
}
